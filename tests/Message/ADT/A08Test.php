...
final class A08Test extends TestCase
{
     ...
     */
    private function parse(array $beforePv1 = [], array $afterPv1 = []): A08
    {
        $lines = ['MSH|^~\\&', 'EVN|A08', 'PID|1', ...$beforePv1, 'PV1|1', ...$afterPv1];
    ...
    {
        // A08 is an Update Patient Information event; the EVN describing it is required and must be directly accessible.
        $this->assertInstanceOf(EVN::class, $this->parse()->getEVN());
    }

    public function testGetPidReturnsThePatientIdentificationSegment(): void
    {
        // An update always concerns a patient; PID access must resolve without an optional check.
        $this->assertInstanceOf(PID::class, $this->parse()->getPID());
    }

    public function testGetPv1ReturnsThePatientVisitSegment(): void
    {
        // The visit being updated is required; PV1 must be reachable as a typed segment.
    ...
    public function testGetPv2ReturnsThePatientVisitAdditionalInformationSegment(): void
    {
        ...
        $message = new A08();
        $message->parse($this->encoding, "MSH|^~\\&\rEVN|A08\rPID|1\rPV1|1\rPV2|1");
    ...
    public function testGetDrgReturnsTheDiagnosisRelatedGroupSegment(): void
    {
        ...
        $message = new A08();
        $message->parse($this->encoding, "MSH|^~\\&\rEVN|A08\rPID|1\rPV1|1\rDRG|1");
